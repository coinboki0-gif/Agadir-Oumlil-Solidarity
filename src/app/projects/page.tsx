...
import { CheckCircle2, Droplet, HeartPulse, GraduationCap, Home, Moon, Landmark, ChevronRight, X, Images } from 'lucide-react';

const hardcodedBeforeImages: Record<string, string[]> = {
  irrigation: [
...
const hardcodedAfterImages: Record<string, string[]> = {
  irrigation: [
...
const colorMap: Record<string, string> = {
  Infrastructure: 'bg-blue-500', Health: 'bg-rose-500',
  Education: 'bg-indigo-500', Solidarity: 'bg-amber-500',
  Housing: 'bg-green-500', Religious: 'bg-teal-600', Water: 'bg-cyan-500',
};
...
const enrichProject = (p: Project) => {
  // gallery من admin يصبح afterImages
  // beforeImages من hardcoded أو من JSON
  const gallery = p.gallery ?? [];
  const before = hardcodedBeforeImages[p.id] ?? p.beforeImages ?? [];
  const after = gallery.length > 0 ? gallery : (hardcodedAfterImages[p.id] ?? p.afterImages ?? []);
...
const ProjectModal = ({ project, onClose }: { project: ReturnType<typeof enrichProject>; onClose: () => void }) => {
  ...
  const hasAfter = project.afterImages.length > 0;
  const hasGallery = hasBefore || hasAfter;
  const imgs = tab === 'before' ? project.beforeImages : project.afterImages;
  const currentImg = imgs.length > 0 ? imgs[imgIndex] : project.mainImage;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative bg-background rounded-3xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="relative h-72 overflow-hidden rounded-t-3xl">
          <img src={currentImg} alt="" className="w-full h-full object-cover transition-all duration-300" />
          ...
          {hasBefore && hasAfter && (
            <div className="absolute top-4 left-1/2 -translate-x-1/2 flex bg-black/40 backdrop-blur-md rounded-full p-1 gap-1">
              <button onClick={() => { setTab('before'); setImgIndex(0); }} className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${tab==='before'?'bg-white text-primary':'text-white/70'}`}>
                {t('قبل','Avant','Before')}
              </button>
              <button onClick={() => { setTab('after'); setImgIndex(0); }} className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all ${tab==='after'?'bg-secondary text-white':'text-white/70'}`}>
                {t('بعد','Après','After')}
              </button>
            </div>
          )}
          ...
          {/* عداد الصور */}
          {imgs.length > 1 && (
            <div className="absolute top-4 right-14 bg-black/50 text-white text-xs px-2.5 py-1 rounded-full">
            ...
            {projects.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filtered.map(project => {
                  const totalPhotos = project.beforeImages.length + project.afterImages.length;
                  return (
                    <div key={project.id} onClick={() => setSelected(project)} className="group bg-card rounded-3xl overflow-hidden border shadow-sm hover:shadow-xl transition-all duration-300 cursor-pointer hover:-translate-y-1">
                      <div className="relative h-52 overflow-hidden">
                        <img src={project.mainImage || project.afterImages[0] || ''} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                        ...
                        {totalPhotos > 0 && (
                          <div className="absolute bottom-3 left-3 bg-black/50 backdrop-blur-sm text-white text-xs px-2.5 py-1 rounded-full flex items-center gap-1">
                            <Images className="w-3 h-3" />
                            {totalPhotos} {t('صورة','photos','photos')}
                          </div>
                        )}
                      </div>
                      <div className="p-6">
